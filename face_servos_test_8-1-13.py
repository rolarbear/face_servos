...
def DetectRedEyes(image, faceCascade, eyeCascade):
	...
	if faces:
		...
		# bounding box of each face and convert it to two CvPoints
			pt1 = (int(x * image_scale), int(y * image_scale))					#8-1-13 change
		...
		cv.SetImageROI(image, (pt1[0],			#8-1-13 change
			pt1[1],								#8-1-13 change
			pt2[0] - pt1[0],					#8-1-13 change
			int((pt2[1] - pt1[1]) * 0.7)))		#8-1-13 change
#		eyes = cv.HaarDetectObjects(image, eyeCascade,		#8-1-13 change
#		cv.CreateMemStorage(0),								#8-1-13 change
#		haar_scale, min_neighbors,							#8-1-13 change
#		haar_flags, (15,15))								#8-1-13 change
